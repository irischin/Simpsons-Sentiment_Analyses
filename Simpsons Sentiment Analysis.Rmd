---
title: "Sentiment Analysis of the Simpsons"
...
I have then subsequently written a loop in which *__each sentence__* is first tokenized into unigrams. The unigrams were then matched to a list of negation words to check whether it contains a negation word. 
...
```{r sentence sentiment scores loop, eval=FALSE}
...
```
```{r reading sentiment score table, echo=FALSE}
#For instances in which you might have run the loop previously and saved the output and therefore do not want to re-run all of the previous code, we can just load the table
corp.sentiment<-read.table("~/Google Drive/Projects/Sentiment Analysis of the Simpsons/corp.sentiment.txt", header=T, sep="\t")
```

We can now sum the individual sentiment scores for each sentence to create a cumulative sentiment score for each episode. Then, we can average those scores to create a mean sentiment score for each season. 
```{r Season sentiment, echo=FALSE, fig.align='center', dpi=600}
corp.sentiment %>%
  group_by(Season, Episode) %>%
  summarise(Sent_total=sum(utterance.afinn)) %>%
  group_by(Season) %>%
  summarise(Average_Sent = mean(Sent_total)) %>%
  ggplot(aes(x=Season, y=Average_Sent)) +
  geom_line()+
  geom_point()+
  ylab("Average Sentiment Score")+
  theme_bw()+
  theme(panel.grid.major=element_blank())+
  geom_vline(xintercept = c(3, 5, 7,9,13), colour="red", linetype = 2)+
...
```{r Comparing joy and anger dictionaries}
...
```{r Quantity of character dialogue}
...
```
```{r Simpsons anger-joy sentiment, echo=FALSE, fig.align='center', dpi=600}
Simpsons.mems.sent %>%
  ggplot(aes(x=Season, y=Sent_prop, group=sentiment, colour=sentiment)) +
  geom_line() +
  geom_point() + 
  scale_x_continuous(breaks=seq(1, 26, 2))+
  scale_y_continuous(limits=c(0,0.5), breaks=seq(0, 0.5, 0.1))+
  ylab("Num. of Anger/Joy Words out of Num. of Utterances") +
  theme_bw()+
  theme(panel.grid.major=element_blank(), 
